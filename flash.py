...
parser = argparse.ArgumentParser(description="feinflash")
parser.add_argument("-d", "--disablemail", action="store_true", help="disable mail")
parser.add_argument("-e", "--env", help="set another virtenv", default="venv")
...
Schritt 2: Flashen
"""
check_output(os.path.join(wd, args.disablemail, 'bin', 'esptool.py') + ' --port /dev/ttyUSB0 chip_id', shell=True)
sensor_flash_status = check_output(os.path.join(wd, args.disablemail, 'bin', 'esptool.py') + ' --port /dev/ttyUSB0  write_flash 0x00000 ' + os.path.join(wd, 'latest.bin'), shell=True)
...
"""
sensor_id = check_output(os.path.join(wd, args.disablemail, 'bin', 'esptool.py') + ' --port /dev/ttyUSB0 chip_id', shell=True)
